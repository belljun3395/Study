...
그리고 이러한 제한으로 인해 다음과 같은 코드도 작성가능해 진다.
...
만약 `<T extends Comparable<T>>` 와 같이 T의 범위를 제한하지 않았더라면 `.compareTo()` 와 같은 메서들 사용하지 못했을 것이다.
...
FruitBox에 선언된 타입 매개변수 T와 제네릭 메서드 sort()에 선언된 타입 매개변수 T는 타입 문자만 같을뿐 서로 다른 것이다.
